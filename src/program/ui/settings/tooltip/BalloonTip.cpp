...
void BalloonTip::showBalloon(const QString& title, const QString& message,
                             ...
                             ShowArrow show_arrow, const int border_width)
{
  hideBalloon();
...
#else
  s_the_balloon_tip = std::unique_ptr<BalloonTip>(new BalloonTip(PrivateTag{}, title, message, parent));
...
#if QT_VERSION < QT_VERSION_CHECK(5, 14, 0)
        const int max_width = QApplication::desktop()->availableGeometry(label).width() / 3;
#else
...
void BalloonTip::updateBoundsAndRedraw(const QPoint& target_arrow_tip_position,
                                       ShowArrow show_arrow, const int border_full_width)
{
    ...
    // position.
#if QT_VERSION < QT_VERSION_CHECK(5, 14, 0)
    const QRect screen_rect = QApplication::desktop()->screenGeometry(target_arrow_tip_position);
#else
