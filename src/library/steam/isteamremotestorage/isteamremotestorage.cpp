...
{
    LOG(LL_TRACE, LCF_STEAM, "%s called with file %s", __func__, pchFile);
...
{
    LOG(LL_TRACE, LCF_STEAM, "%s called with file handke %ull and size %d", __func__, writeHandle, cubData);
...
{
    LOG(LL_TRACE, LCF_STEAM, "%s called with file handke %ull", __func__, writeHandle);
...
const char *ISteamRemoteStorage_GetFileNameAndSize( void* iface, int iFile, int *pnFileSizeInBytes )
{
    ...
    *pnFileSizeInBytes = ISteamRemoteStorage_GetFileSize(iface, dir->d_name);
    LOG(LL_DEBUG, LCF_STEAM, "   return file %s and size %d", dir->d_name, *pnFileSizeInBytes);
