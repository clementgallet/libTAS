...
/* Override */ long int random (void) __THROW
{
    ...
    LOGTRACE(LCF_RANDOM, "%s call %d, returning %ld", __func__, count++, ret);
    return ret;
}

/* Override */ void srandom (unsigned int seed) __THROW
{
    LOGTRACE(LCF_RANDOM, "%s call with seed %u", __func__, seed);
...
{
    LOGTRACE(LCF_RANDOM, "%s call with seed %ld", __func__, seedval);
    RETURN_NATIVE(srand48, (seedval), nullptr);
}

/* Override */ unsigned short int *seed48 (unsigned short int seed16v[3]) __THROW
{
    LOGTRACE(LCF_RANDOM, "%s call with seed %d %d %d", __func__, seed16v[0], seed16v[1], seed16v[2]);
