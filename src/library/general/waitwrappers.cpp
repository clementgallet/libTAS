...
/* Override */ int poll (struct pollfd *fds, nfds_t nfds, int timeout)
{
    ...
    LOGTRACE(LCF_WAIT, "%s call with %d fds and timeout %d", __func__, nfds, timeout);
...
/* Override */ int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout)
{
    ...
    LOGTRACE(LCF_SLEEP, "%s call - sleep for %d.%06d sec", __func__, timeout->tv_sec, timeout->tv_usec);
	...
	const struct timespec *timeout, const sigset_t *sigmask)
{
    ...
    LOGTRACE(LCF_SLEEP, "%s call - sleep for %d.%09d sec", __func__, timeout->tv_sec, timeout->tv_nsec);
...
/* Override */ int epoll_wait (int epfd, struct epoll_event *events, int maxevents, int timeout)
{
    ...
    LOGTRACE(LCF_SLEEP, "%s call with timeout %d", __func__, timeout);
